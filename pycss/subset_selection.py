...
def compute_imputed_moments(X, MLE):

    """
    Computes imputed first and second moments of the data matrix `X` which has missing data values
    according to the spherical or diagonal Gaussian PCSS model, using the MLEs in the dictionary 
    `MLE`.  
  
    Parameters
    ----------
    X : np.array
        A `(n, p)`-shaped data matrix with missing values. The missing values should be encoded as 
        `np.nan` entries. 
	MLE : Dict[str, np.array]
        A dictionary containing the maximum likelihood estimates. The keys for the spherical Gaussian
        PCSS model are `'mu_MLE'`, `'S_MLE'`, `'C_MLE'`, `'W_MLE'` and `'sigma_sq_MLE'`. The keys for the diagonal
	...
	-------
    m : np.array
        The `(n, p)`-shaped array containing the imputed first moments of the data matrix. 
    Omega : np.array
        The `(n, p, p)`-shaped array containing the imputed second moments of the data matrix.  
	"""
    ...
    mu_MLE_S = MLE['mu_MLE'][S]
    mu_MLE_S_comp = MLE['mu_MLE'][S_comp]

    k = len(S)
    range_p = np.arange(p)
    range_k = np.arange(k)
    num_other = p-k
    range_num_other = np.arange(num_other)
    ...
    if np.any(np.isnan(X_S)):
        ...
        Sigma_MLE[:k, k:] = Sigma_MLE[k:, :k].copy().T
        Sigma_MLE[k:, k:] = MLE['W_MLE'] @ Sigma_MLE[:k, k:] 
        if noise == 'sph':
    ...
    for i in range(n):
        ...
        other_missing = np.where(np.isnan(x_S_comp))[0]
        other_not_missing = np.where(~np.isnan(x_S_comp))[0]
        num_other_missing = len(other_missing)

        missing = np.concatenate([subset_missing, other_missing + k])
        ...
        if len(subset_missing) == 0:
            ...
            m_missing = MLE['W_MLE'][other_missing, :] @ (x_S - mu_MLE_S) + mu_MLE_S_comp[other_missing]
            Omega_missing = np.outer(m_missing, m_missing)

            if noise == 'sph':
                np.fill_diagonal(Omega_missing, np.diag(Omega_missing) + MLE['sigma_sq_MLE'])
            if noise == 'diag':
                np.fill_diagonal(Omega_missing, np.diag(Omega_missing) + MLE['D_MLE'][other_missing])
      
        else:

            Sigma_perm = np.concatenate([not_missing, missing])
            mu_MLE_ordered = MLE['mu_MLE'][Sigma_perm]
            perm_in_place(Sigma_MLE, range_p, Sigma_perm)
 
            subset_perm = np.concatenate([subset_missing, subset_not_missing])
            other_perm = np.concatenate([other_not_missing, other_missing])

            C_MLE = MLE['C_MLE'].copy()
            W_MLE = MLE['W_MLE'].copy()
        
            perm_in_place(C_MLE, range_k, subset_perm)
            perm_in_place(W_MLE, range_k, subset_perm, row=False)
            perm_in_place(W_MLE, range_num_other, other_perm, col=False)
        
            if noise == 'sph':
                top_left_block, bottom_left_block, bottom_right_block  = compute_Sigma_MLE_chol(MLE = {'mu_MLE': None,
                                                         'S_MLE': S,
                                                         'C_MLE': C_MLE,
                                                         'W_MLE': W_MLE[:num_other - num_other_missing, :],
                                                         'sigma_sq_MLE': MLE['sigma_sq_MLE']})
            if noise == 'diag':
                top_left_block, bottom_left_block, bottom_right_block  = compute_Sigma_MLE_chol(MLE = {'mu_MLE': None,
                                                         'S_MLE': S,
                                                         'C_MLE': C_MLE,
                                                         'W_MLE': W_MLE[:num_other - num_other_missing, :],
                                                         'D_MLE': MLE['D_MLE'].copy()[other_perm][:num_other - num_other_missing]})                                        

            len_top = top_left_block.shape[0]
            len_total = np.sum(np.shape(bottom_left_block))
            Sigma_MLE_not_missing_chol = np.zeros((len_total, len_total))
            Sigma_MLE_not_missing_chol[:len_top, :len_top] = top_left_block
            Sigma_MLE_not_missing_chol[len_top:, :len_top] = bottom_left_block
            np.fill_diagonal(Sigma_MLE_not_missing_chol[len_top:, len_top:], bottom_right_block)

            for _ in range(num_subset_missing):
                Sigma_MLE_not_missing_chol = update_cholesky_after_removing_first(Sigma_MLE_not_missing_chol)

            Sigma_MLE_not_missing_inv = solve_with_cholesky(Sigma_MLE_not_missing_chol, np.eye(num_not_missing))
            Sigma_MLE_missing_not_missing = Sigma_MLE[(p - num_missing):p , :num_not_missing]
            temp =  Sigma_MLE_missing_not_missing @ Sigma_MLE_not_missing_inv
            m_missing = temp @ (x_not_missing - mu_MLE_ordered[:num_not_missing]) + mu_MLE_ordered[num_not_missing:]
            Omega_missing = Sigma_MLE[num_not_missing:, num_not_missing:] -  temp @ Sigma_MLE_missing_not_missing.T + np.outer(m_missing, m_missing)
         
            perm_in_place(Sigma_MLE, range_p, np.argsort(Sigma_perm))
        ...
        Omega_perm = np.zeros(p).astype(int)
        Omega_perm[missing] = np.arange(num_not_missing, p)
        Omega_perm[not_missing] = np.arange(num_not_missing)
        perm_in_place(Omega[i, :, :], range_p, Omega_perm)

    revert_perm = np.argsort(np.concatenate([S, S_comp]))
    perm_in_place(m, range_p, revert_perm, row=False)
    perm_in_place(Omega, range_p, revert_perm)
